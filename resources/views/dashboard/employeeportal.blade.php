...
@section('content')
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 hidden" id="updateDetailsModal" role="dialog" aria-labelledby="updateModalTitle" aria-modal="true">
        <div class="bg-white rounded-xl w-full max-w-2xl modal-content">
            ...
            <div class="p-6">
                <form id="updateDetailsForm" action="{{ route('employee.portal.update') }}" method="POST">
                    @csrf
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 hidden" id="viewPayslipsModal" role="dialog" aria-labelledby="payslipsModalTitle" aria-modal="true">
        <div class="bg-white rounded-xl w-full max-w-4xl modal-content">
            ...
            <div class="p-6">
                ...
                <table class="w-full text-left">
                    ...
                    <tbody>
                        @foreach($payslips as $payslip)
                            <tr>
                                <td class="py-2 px-4 text-sm text-gray-600">{{ $payslip->period ?? $payslip->pay_period ?? 'N/A' }}</td>
                                <td class="py-2 px-4 text-sm text-gray-600">{{ number_format($payslip->gross_salary ?? 0, 2) }}</td>
                                <td class="py-2 px-4 text-sm text-gray-600">{{ number_format($payslip->net_salary ?? 0, 2) }}</td>
                                <td class="py-2 px-4 text-sm">
                                    <span class="status-badge {{ ($payslip->status ?? 'Pending') == 'Processed' ? 'status-paid' : 'status-pending' }}">
                                        {{ $payslip->status ?? 'Pending' }}
                                    </span>
                                </td>
                                <td class="py-2 px-4 text-sm">
                                    <a href="{{ route('employee.payslip.download', $payslip->id) }}" class="text-blue-600 hover:text-blue-800">
                ...
                @else
                    <p class="text-gray-500 text-center py-8">No payslips available yet.</p>
                @endif
